...
func NewSearchService[T any, K any, F any](db *sql.DB, repository SearchRepository[T, K, F], opts ...string) *SearchService[T, K, F] {
	txKey := "tx"
	if len(opts) > 0 && len(opts[0]) > 0 {
		txKey = opts[0]
	}
	return &SearchService[T, K, F]{db, repository, txKey}
}
