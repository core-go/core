...
func NewDiffHandler(diffService DiffService, modelType reflect.Type, resource string, logError func(context.Context, string), writeLog func(context.Context, string, string, bool, string) error, config *DiffModelConfig, option ...int) *DiffHandler {
	...
	return &DiffHandler{Log: writeLog, DiffService: diffService, ModelType: modelType, IdNames: idNames, Indexes: indexs, Resource: resource, Offset: offset, Config: config, Error: logError}
}
...
func (c *DiffHandler) Diff(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if err != nil {
			...
			if c.Config == nil {
				Succeed(w, r, http.StatusOK, result, c.Log, c.Resource, "diff")
			} else {
				...
				Succeed(w, r, http.StatusOK, m, c.Log, c.Resource, "diff")
			}
		}
	}
}
